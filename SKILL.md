...
1. **创建场景文件**：`scenarios/s06_<描述>.py`
2. **继承 BaseScenario**：实现五个生命周期方法 + `emergency_restore()`
3. **命名规范**：模块导出 `Scenario` 类，文件名以 `s<编号>_` 开头
4. **注册**：`run_tests.py` 会自动发现 `scenarios/` 下的 `s*` 模块，无需手动注册
